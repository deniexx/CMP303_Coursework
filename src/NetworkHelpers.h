...
namespace NetworkHelpers
{
    static void UpdatePhysicsForEntity(const PhysicsUpdateMessage& message)
    {
        ...
        if (level->HasComponent<TransformComponent>(message.m_playerID))
		{
			...
			sf::Vector2f desiredTrans(message.m_x, message.m_y);

			if (Length(trans - desiredTrans) > ACCEPTABLE_POSITION_DELTA * 2)
			{
				MovementComponent& movementComp = level->GetComponent<MovementComponent>(message.m_playerID);
				movementComp.m_interpolationTarget = desiredTrans;
				movementComp.m_startingInterpPosition = trans;
				movementComp.m_interpAlpha = 0.f;
			}
		}
    }
